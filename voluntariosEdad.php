						    ...
						    if(implode($perm) == '6'){
						    	echo "<div id='divArribaLista'>
										<div id='opcionArribaLista'>
											<a id='opcionArribaLista' href='regVoluntario.php'>Registrar Voluntario</a>
										</div>
									</div>";
						    }
						    if(implode($perm) == '7'){
					...
					?>
						...
						<table id='tablaOutsideBordes'> <!--id="tablaBordes"-->
							...
							<?php
								...
								$edadQuery= "SELECT * FROM edadesVoluntarios;";
								$res= mysqli_query($db,$edadQuery);

								for ($i=0; $i < $res->num_rows; $i++) { 
									$voluntarios= $res->fetch_assoc();
									$nombres= $voluntarios['nombres'];
									$apellidoPat= $voluntarios['apellidoPat'];
									$fechaDeNac= $voluntarios['fechaDeNac'];
									$edad= $voluntarios['edad'];


									echo "<tr >";
									echo "<td >$nombres</td>";
									echo "<td >$apellidoPat</td>";
									echo "<td >$fechaDeNac</td>";
									echo "<td >$edad</td>";
									echo "</tr>";
